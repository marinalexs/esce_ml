...
from esce.fit import fit, get_existing_scores
...
def test_fit(tmpdir):
    ...
    split = {"idx_train": list(range(0, 60)), "idx_val": list(range(60, 80)), "idx_test": list(range(80, 100)), 
             "samplesize": 100, "seed": 42}
    ...
    fit(features_path, targets_path, split_path, scores_path, model_name, grid_path, existing_scores_path_list)
...
def test_fit_with_existing_scores(tmpdir):
    ...
    df_existing_scores = pd.DataFrame([
        {'alpha': 0.1, 'acc_train': EXISTING_ACC, 'acc_val': EXISTING_ACC, 'acc_test': EXISTING_ACC, 'n': 100, 's': 42}
    ])
