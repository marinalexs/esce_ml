...
def generate_random_split(
    ...
    split = {'idx_train': idx_originial[idx_train],
             'idx_val': idx_originial[idx_val], 'idx_test': idx_originial[idx_test]}
...
def generate_matched_split(
    ...
    split = generate_random_split(
        y, n_train//2, n_val//2, n_test//2, do_stratify, seed, np.logical_and(mask, y > 0))
...
def write_splitfile(features_path, targets_path, split_path, sampling_path, sampling_type, n_train, n_val, n_test, seed, stratify=False):
    ...
    if sampling_type == 'raw':
        ...
        matching = False
        idx_undersampled = RandomUnderSampler(
            random_state=seed).fit_resample(idx_all[xy_mask], y[xy_mask])
...
n_val = min(round(n_train*snakemake.params.val_test_frac),
            snakemake.params.val_test_max)
n_test = min(round(n_train*snakemake.params.val_test_frac),
             snakemake.params.val_test_max)
