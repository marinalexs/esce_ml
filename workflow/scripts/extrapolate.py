...
def extrapolate(
    ...
    for n in sorted(df['n'].unique()):
        ...
        y_sem.append(df[df['n'] == n][metric].std() /
                     np.sqrt(result['n_seeds']))
    p_mean, _ = scipy.optimize.curve_fit(lambda t, a, b, c: a*t**(-b)+c,  x, y_mean,
                                         sigma=None, maxfev=5000, p0=(-1, 0.01, 0.7), bounds=((-1, 0, 0), (0, 1, 1)))
    ...
    for _ in range(repeats):
        ...
        for i, n in enumerate(x):
            y_sample = df[df['n'] == n][metric].sample(
                frac=1, replace=True).mean()
            y.append(y_sample)
            y_bootstrap[i].append(y_sample)
        try:
            p_, _ = scipy.optimize.curve_fit(lambda t, a, b, c: a*t**(-b)+c,  x, y,
                                             sigma=None, maxfev=5000, p0=(-1, 0.01, 0.7), bounds=((-1, 0, 0), (0, 1, 1)))
    ...
    result['chi2'] = sum(
        (y_mean-(p_mean[0]*x**(-p_mean[1])+p_mean[2]))**2/y_sem**2) / (len(x)-len(p_mean))
    result['mu'], result['sigma'] = np.mean(
        (y_mean-(p_mean[0]*x**(-p_mean[1])+p_mean[2]))/y_sem), np.std((y_mean-(p_mean[0]*x**(-p_mean[1])+p_mean[2]))/y_sem)
...
extrapolate(snakemake.input.scores, snakemake.output.stats,
            snakemake.output.bootstraps, snakemake.params.bootstrap_repetitions)
